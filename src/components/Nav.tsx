...
export function Nav() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      // Hide/show based on scroll direction (only on mobile)
  ...
  return (
    ...
    <nav
      className={cn(
        "fixed z-50 transition-all duration-500 ease-[cubic-bezier(0.32,0.72,0,1)] will-change-[width,height,top,border-radius,transform]",
        // Mobile: full-width bar, Desktop: centered floating
        "left-0 right-0 md:left-1/2 md:right-auto md:-translate-x-1/2",
        // Mobile styles - glassy gradient (more opaque at top, translucent at bottom)
        "top-0 w-full h-14 border-b border-border md:border-b-0",
        "backdrop-blur-md md:backdrop-blur-none",
        "bg-gradient-to-b from-background/90 via-background/70 to-background/50 md:bg-transparent",
        // Desktop styles
        scrolled
          ? "md:top-4 md:w-[calc(100%-2rem)] md:max-w-6xl md:rounded-2xl md:border md:shadow-lg md:backdrop-blur-xl md:h-16 md:glass-strong"
          : "md:top-0 md:glass-subtle md:border-transparent md:h-20",
        // Hide on scroll down (mobile only)
        hidden && !mobileMenuOpen ? "-translate-y-full md:translate-y-0" : "translate-y-0"
      )}
      onMouseLeave={() => setActiveDropdown(null)}
    >
      <div className={cn(
        "mx-auto h-full px-4 sm:px-6",
        scrolled ? "md:max-w-7xl md:px-6 lg:px-8" : "md:w-[1310px]"
      )}>
        <div className="flex items-center justify-between h-full">
          ...
          <button
            className="md:hidden z-[60] p-2 text-ink-primary"
